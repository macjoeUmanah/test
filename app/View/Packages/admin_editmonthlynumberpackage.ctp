...
<?php echo $this->Form->create('Package',array('style'=>'width:98%'));?>
	<fieldset>
		<legend><?php echo('Edit Monthly Number Package'); ?></legend>
	<?php
...
'Zimbabwe'=>'Zimbabwe');
	    ...
	    foreach($users as $user){ 
				...
				if ($package['MonthlyNumberPackage']['username'] == $user['username']) {
				    echo ' selected';
				}
				echo '>'.$user["username"];
				echo '</option>';
	    ...
	    echo '</select></div>';
		echo $this->Form->input('MonthlyNumberPackage.status',array('type'=>'select','options'=>$Option));
