...
$.extend(DayGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		if (!event.allDay && seg.isStart) {
			...
			var viewlink = htmlEscape(siteurl+'/appointments/events_view/'+'/'+event.event_id);
			titleHtml =
			'<span class="fc-title" onClick="viewfunction(\''+htmlEscape(viewlink)+'\');">' +
			(htmlEscape(event.title.substring(0,30) || '') || '&nbsp;') + // we always want one line of height
			'</span>';
			
			var deletelink = htmlEscape(siteurl+'/appointments/events_delete/'+event.event_id);
			return '<div class="fc-content fc-day-grid-event fc-event fc-start fc-end" style="background-color:'+event.bgcolor+';border-color:'+event.bgcolor+'">' +
			(isRTL ?
			titleHtml + ' ' + timeHtml : // put a natural space in between
			timeHtml + ' ' + titleHtml   //
			) +
			'<div style="float:right;"><span style="color:red;float:left;padding-left: 4px;"><span onClick="editfunction(\''+htmlEscape(event.url)+'\');"><img src="'+siteurl+'/appointment/fullcalendar/pencil.png"></span></span><span style="color:red;float:left;padding-left: 4px;"><span onClick="confirmfunction(\''+deletelink+'\');"><img src="'+siteurl+'/appointment/fullcalendar/cross.png"></span></span></div><div class="clearfix"></div></div>' +
			(isResizable ?
...
$.extend(TimeGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		return '<span style="font-size:12px !important; background-color:'+event.bgcolor+';border-color:'+event.bgcolor+'" onClick="editfunction(\''+event.url+'\');" class="' + classes.join(' ') + '"' +
			(skinCss ?
				' style="background-color:'+event.bgcolor+';border-color:'+event.bgcolor+'"' :
				''
				) +
			'>' +
